...
- **TanStack Start:** Ensure React 19+ is installed. TanStack Start bundles the necessary React features internally. `ViewTransition` works out of the box with `startTransition` and `Suspense`.
- **React 19 Required:** `ViewTransition` requires React 19+ (`react@^19 react-dom@^19`).
...
For TanStack Start-specific steps, see `references/tanstack-start.md`.
